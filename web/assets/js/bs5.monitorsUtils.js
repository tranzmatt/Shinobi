...
function drawPluginElements(){
    var numberOfPluginsConnected = Object.keys(connectedDetectorPlugins).length;
    if(numberOfPluginsConnected === 0){
    ...
    $.each(onDetectorPluginChangeExtensions,function(n,extender){
        extender(numberOfPluginsConnected)
    })
}
function setFieldVisibilityForSelectors(formElement){
    var listToShow = []
...
function getMonitorEditFormFields(editorForm){
...
function setFieldVisibilityConditions(editorForm){
...
function setFieldVisibility(editorForm){
    setFieldVisibilityForSelectors(editorForm)
    setFieldVisibilityConditions(editorForm)
}
function loadMonitorGroupTriggerList(monitor,triggerTagsInput){
    var monitorTriggerTags = (monitor && monitor.details.det_trigger_tags ? monitor.details.det_trigger_tags : '').split(',')
